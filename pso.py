...
st.write("PSO for traffic signal timing using waiting time & average speed.")
...
if uploaded_file is not None:
    ...
    def compute_delay(green_times):
        prop = green_times / green_times.sum()  # fraction of total cycle
        avg_waiting = np.mean(waiting_time)
        avg_speed = np.mean(average_speed)
        # Objective: minimize waiting time and low speed
        delay = np.sum(avg_waiting * prop) + np.sum((1 / (avg_speed + 1e-3)) * prop)
        return delay
    ...
    if st.button("Run PSO Optimization"):
        ...
        - **Optimization Quality:** Delay minimized using waiting time & average speed  
        - **Computational Efficiency:** Low execution time  
        - **Interpretability:** Green times for each phase are clear
        """)
        st.header("Conclusion")
        st.markdown("""
        This Streamlit-based PSO system demonstrates how **Particle Swarm Optimization** can find 
        effective traffic signal timings using waiting time and average speed. Users can adjust 
        parameters, visualize convergence, and obtain optimized phase timings.
        """)
